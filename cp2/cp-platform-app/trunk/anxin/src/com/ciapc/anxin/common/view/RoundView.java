...
public class RoundView extends ImageView {
	private static String TAG = "RoundView";
	...
	protected void onDraw(Canvas canvas) {
		try {
			...
			if (null == bitmap)
				return;
			// bitmap = ImageUtil.imageZoom(bitmap);
	...
	public Bitmap getCroppedRoundBitmap(Bitmap bmp, int radius) {
		...
		squareBitmap = bmp;
		Matrix m = new Matrix();
		float w = squareBitmap.getWidth();
		float h = squareBitmap.getHeight();
		m.postScale((float) (60.0 / w), (float) (60.0 / h));
		squareBitmap = Bitmap.createBitmap(squareBitmap, 0, 0,
				squareBitmap.getWidth(), squareBitmap.getHeight(), m, true);// 用距阵的方式缩放
	...
	private void drawCircleBorder(Canvas canvas, int radius, int color) {
		try {
			...
			canvas.drawCircle(defaultWidth / 2, defaultHeight / 2, radius,
					paint);
		} catch (Exception e) {
			e.printStackTrace();
		}
